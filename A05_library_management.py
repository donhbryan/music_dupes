...
class MusicLibraryManager:
    ...
    def _setup_database(self):
        ...
        # Added acoustid_id column to track the normalized ID
        self.cur.execute(
        ...
        # Add index on acoustid_id for fast duplicate lookups
        self.cur.execute(
    ...
    def _calculate_quality(self, file_path):
        ...
        try:
            time.sleep(0.15)  # Pause to let file system settle
    ...
    def _handle_deduplication(self, path, acoustid_id, quality):
        """
        Check for duplicates using the AcoustID UUID (which survives format conversion).
        Returns True if current file is the winner.
        """
        # Look for ANY file with the same AcoustID, regardless of its specific raw fingerprint
        self.cur.execute(
        ...
        if existing:
            ...
            if quality["score"] > (existing_score := existing_score if existing_score else 0):
    ...
    def process_library(self):
        ...
        for i, path in enumerate(files):
            ...
            try:
                ...
                # Added 'tracks' to meta to ensure track list is populated
                ...
                # Get the AcoustID (Track ID) from the best match
                ...
                # NOW check for duplicates using the ID, not the raw fingerprint
                if not self._handle_deduplication(path, current_acoustid_id, quality):
                    continue
                ...
                # Correctly finding track number by matching recording ID inside the release tracks
                track_num = 1
                disc_num = 1

                # Iterate through mediums (Discs)
                found_track = False
                for medium in rel.get("mediums", []):
                    # Iterate through tracks on this medium
                    for track in medium.get("tracks", []):
                        # Usually track has 'recording' dict with 'id'
                        # Or sometimes 'id' is direct. AcoustID structure usually nests recording under track.
                ...
                self.cur.execute(
                    ...
                    (
                        final_path,
                        fingerprint,
                        current_acoustid_id,  # Store the ID!
                        meta["title"],
                        meta["track_no"],
                        meta["disc_no"],
                        quality["format"],
