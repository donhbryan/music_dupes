...
class MusicLibraryManager:
    def __init__(self, config_file="library_management_config.json"):
        ...
        # FIXED: Using RLock (Reentrant Lock) to prevent self-deadlocks 
        # when methods call other methods that also require the database lock.
    ...
    def _setup_database(self):
        with self.db_lock:
            ...
            # Increase busy timeout so SQLite threads queue instead of locking
    ...
    def _get_audio_hash(self, filepath):
        ...
        try:
            # FIXED: capture_output=True prevents OS pipe buffer from filling and freezing
    ...
    def _get_fingerprint(self, filepath):
        try:
            # -length 120 speeds up processing massively
            proc = subprocess.run(
    ...
    def _play_audio(self, file_path):
        ...
        commands.extend([["ffplay", "-nodisp", "-autoexit", "-hide_banner", "-loglevel", "quiet", file_path], ["mpv", "--no-video", "--quiet", file_path], ["cvlc", "--play-and-exit", "--quiet", file_path]])
    ...
    def _prompt_user_selection(self, file_path, candidates):
        ...
        try:
            while True:
                ...
                prompt_options.extend(["(R)eplay", "(0) Skip", "(Q)uit"])

                choice = input(f"Select Album # (1-{len(candidates)}, comma-separated), " + ", ".join(prompt_options) + ": ").lower()
    ...
    def process_library(self):
        ...
        # Capped to 8 threads to strictly prevent local disk IO starvation
    ...
    def resolve_ambiguous_files(self):
        ...
        print(f"\n--- Phase 2: Resolving {len(rows)} Ambiguous Files ---")
        for row in rows:
            ...
            if result == "quit":
                ...
                for idx, selected_match in enumerate(result): self._process_match_for_file(path, acoustid_id, fingerprint, quality, audio_hash, selected_match, idx == len(result) - 1)
    ...
    def _process_match_for_file(self, path, current_acoustid_id, fingerprint, quality, audio_hash, selected_match, is_last_item):
        ...
        with self.db_lock:
            ...
            # Inline the index update to ensure it completes atomically without separate locking loops
